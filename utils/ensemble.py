...
parser.add_argument('-i1',
                    '--input1',
                    type=str,
                    help='Swin-L HTC Output submission file (.csv format)')
parser.add_argument('-i2',
                    '--input2',
                    type=str,
                    help='Swin-B UperNet Output submission file (.csv format)')
parser.add_argument('-o',
                    '--output',
                    type=str,
                    help='Final ensembled submission file (.csv format)')
args = parser.parse_args()

# model 1: swin-L HTC model, Instance segmentation
csv_1 = pd.read_csv(args.input1)
# model 2: swin-B UperNet
csv_2 = pd.read_csv(args.input2)

# submission file의 PredictionString이 한 str에 한 image의 모든 pixel의 category를 담고있다.
# 따라서 pixel by pixel로 split 후 array로 저장
pred_1 = np.array(
    [csv_1['PredictionString'][i].split() for i in range(len(pred_1))],
    dtype='int8')
pred_2 = np.array(
    [csv_2['PredictionString'][i].split() for i in range(len(pred_1))],
    dtype='int8')
